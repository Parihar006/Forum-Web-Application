...
<body>
    ...
    <div class="container my-2 mb-4" id="ques">
        ...
        while($row = mysqli_fetch_assoc($result)){
            $thredTitle = $row['thread_title'];
            $threadDesc = $row['thread_description'];
            $noResults=false;
            $threadId = $row['thread_id'];
            $threadUserId = $row['thread_user_id'];
            echo '<div class="result">
                    <h3><a class="text-dark" href="thread.php?threadid=' .$threadId. '&thread_user_id=' .$threadUserId. '">' .$thredTitle. '</a></h3>
                    <p>' .$threadDesc. '</p>
                </div>';
        }
